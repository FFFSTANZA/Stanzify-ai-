        ...
        <div className="flex items-center justify-center gap-4">
          {steps.map((step, index) => (
            <div key={index} className="flex items-center">
              <div 
                className="flex flex-col items-center max-w-xs p-6 rounded-xl bg-white shadow-lg animate-in fade-in zoom-in duration-700"
      ...
      {layout === 'vertical' && (
        <div className="max-w-2xl mx-auto space-y-6">
          {steps.map((step, index) => (
            <div 
              key={index}
              className="flex gap-6 items-start animate-in fade-in slide-in-from-left duration-700"
              style={{ animationDelay: `${index * 200}ms` }}
            >
              <div 
